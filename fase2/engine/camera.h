...
// Gerencia a câmera 3D da cena, controlando posição, orientação e projeção.
...
private:
    ...
    // Converte as coordenadas esféricas (alpha, beta, radius) para cartesianas.
...
// - Projeção: FOV=60°, near=1, far=1000
    ...
    // Construtor parametrizado da câmera.
// - px, py, pz Posição inicial da câmera
// - lx, ly, lz Ponto para o qual a câmera está olhando
// - ux, uy, uz Vetor "para cima"
// - fovVal: Campo de visão em graus
...
// - far: Plano de clipping distante
    ...
    // Retorna a coordenada X da posição da câmera
    float getPosX() const { return posX; }
    // Retorna a coordenada Y da posição da câmera
    float getPosY() const { return posY; }
    // Retorna a coordenada Z da posição da câmera
    float getPosZ() const { return posZ; }
    
    // Retorna a coordenada X do ponto lookAt
    float getLookAtX() const { return lookAtX; }
    // Retorna a coordenada Y do ponto lookAt
    float getLookAtY() const { return lookAtY; }
    // Retorna a coordenada Z do ponto lookAt
    float getLookAtZ() const { return lookAtZ; }
    
    // Retorna a coordenada X do vetor "para cima"
    float getUpX() const { return upX; }
    // Retorna a coordenada Y do vetor "para cima"
    float getUpY() const { return upY; }
    // Retorna a coordenada Z do vetor "para cima"
    float getUpZ() const { return upZ; }
    
    // Retorna o campo de visão em graus
    float getFov() const { return fov; }
    // Retorna a distância do plano de clipping próximo
    float getNearPlane() const { return nearPlane; }
    // Retorna a distância do plano de clipping distante
    float getFarPlane() const { return farPlane; }
    
    // Retorna o ângulo horizontal (azimute) em radianos
    float getAlpha() const { return alpha; }
    // Retorna o ângulo vertical (elevação) em radianos
    float getBeta() const { return beta; }
    // Retorna a distância da câmera ao ponto lookAt
    float getRadius() const { return radius; }
    
    

    // Define a posição da câmera.
// - x, y, z Novas coordenadas cartesianas da câmera
// Nota: Recalcula automaticamente as coordenadas esféricas
    void setPosition(float x, float y, float z);
    
    // Define o ponto para o qual a câmera está olhando.
// - x, y, z Novas coordenadas do ponto lookAt
// Nota: Recalcula automaticamente as coordenadas esféricas
    void setLookAt(float x, float y, float z);
    
    // Define o vetor "para cima" da câmera.
// - x, y, z Componentes do novo vetor up
// Nota: Não recalcula coordenadas esféricas (parâmetro independente)
    void setUp(float x, float y, float z);
    
    // Define os parâmetros de projeção perspectiva.
// - fovVal: Campo de visão em graus
// - near: Distância do plano de clipping próximo
// - far: Distância do plano de clipping distante
    void setProjection(float fovVal, float near, float far);
    
   
    
    // Calcula as coordenadas esféricas a partir da posição cartesiana.
// Converte a diferença entre a posição atual e o ponto lookAt
// para coordenadas esféricas (alpha, beta, radius).
// Nota: Chamada automaticamente pelos setters de posição e lookAt
    void calculateSphericalCoords();
    
    
    // Rotaciona a câmera para a esquerda (diminui alpha).
// A rotação é realizada em torno do eixo vertical (Y) passando pelo ponto lookAt.
    void rotateLeft();
    
    // Rotaciona a câmera para a direita (aumenta alpha).
// A rotação é realizada em torno do eixo vertical (Y) passando pelo ponto lookAt.
    void rotateRight();
    
    // Rotaciona a câmera para cima (aumenta beta).
// A rotação máxima é limitada a π/2 para evitar inversão da câmera.
    void rotateUp();
    
    // Rotaciona a câmera para baixo (diminui beta).
// A rotação mínima é limitada a -π/2 para evitar inversão da câmera.
    void rotateDown();
    
    // Aproxima a câmera do ponto lookAt (diminui o radius).
// O radius é limitado a um mínimo de 0.1 para evitar valores inválidos.
    void zoomIn();
    
    // Afasta a câmera do ponto lookAt (aumenta o radius).
    void zoomOut();
    
    
    // Aplica a transformação da câmera no contexto OpenGL.
